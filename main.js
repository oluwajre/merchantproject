...
Alpine.data('sidebar', (active)=>({
  ...
  openDropdown: '',
  menuList: [
      {name: 'Dashboard', icon: icons.dashboard, dropDown: [], link: './login.html'},
      {menuHeader: 'Notification', name: 'Data Warehouse', icon: icons.dataWarehouse, dropDown: [], link: 'https://www.creditswitch.com/', alert: '0'},
      {menuHeader: 'Report', name: 'Switching Controls', icon: icons.switchingControls,
        dropDown: [
            {name: 'Switching Report', icon: icons.subMenu, link: 'https://www.creditswitch.com/'},
  ...
  toggleDropdown(item, index) {
      ...
      if(item.availableSubDropDown) {
        ...
        console.log('sub dropdown reached')
      }else {
        this.openDropdown = this.openDropdown === index ? '' : index;
        console.log('normal dropdown reached')
      }
  },

  isActive(item) {
    if (item.name === active) return true;
    return item.dropDown?.some(child => child.name === active);
  },
  toggleSideBar() {
      if (this.showSideBar) {
